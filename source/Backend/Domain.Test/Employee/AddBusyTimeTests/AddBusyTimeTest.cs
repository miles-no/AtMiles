...
namespace no.miles.at.Backend.Domain.Test.Employee.AddBusyTimeTests
{
    ...
    public class AddBusyTimeTest : EventSpecification<AddBusyTime>
    {
        ...
        {
            return new AddBusyTime(CompanyId, EmployeeId, Start1, End1, Percentage1, Comment1, DateTime.UtcNow, new Person(EmployeeId, NameService.GetName(EmployeeFirstName, EmployeeLastName)), _correlationId, Constants.IgnoreVersion);
        }
        ...
        protected override IEnumerable<Event> Expect()
        {
            var events = new List<Event>
                {
                    new BusyTimeAdded(CompanyId, CompanyName, EmployeeId, NameService.GetName(EmployeeFirstName, EmployeeLastName), _busyTimeId, Start1, End1, Percentage1, Comment1, _timestamp, new Person(EmployeeId, NameService.GetName(EmployeeFirstName, EmployeeLastName)),_correlationId)
                };
            return events;
        }
    }
