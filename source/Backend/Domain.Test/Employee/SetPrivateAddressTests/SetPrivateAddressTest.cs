...
namespace no.miles.at.Backend.Domain.Test.Employee.SetPrivateAddressTests
{
    ...
    public class SetPrivateAddressTest : EventSpecification<SetPrivateAddress>
    {
        ...
        [Test]
        public async void set_private_address()
        {
            await Setup();
        }
        ...
        public override IEnumerable<Event> Expect()
        {
            var events = new List<Event>
                {
                    new PrivateAddressSet(CompanyId, CompanyName, EmployeeId, NameService.GetName(EmployeeFirstName, EmployeeLastName), _privateAddress, _timestamp, new Person(EmployeeId, NameService.GetName(EmployeeFirstName, EmployeeLastName)),_correlationId)
                };
            return events;
        }
    }
