...
namespace Contact.Domain.Test.Employee.AddEmployeeTests
{
    ...
    public class AddEmployeeUnknownAdmin : EventSpecification<AddEmployee>
    {
        ...
        {
            var cmd = new AddEmployee(CompanyId, OfficeId, employeeGlobalId, employeeLoginId, EmployeeFirstName, EmployeeLastName, EmployeeDateOfBirth, string.Empty, string.Empty, string.Empty, null, null, null, DateTime.UtcNow, new Person(AdminId, NameService.GetName(AdminFirstName, AdminLastName)), _correlationId, 2);
            return cmd;
        }
