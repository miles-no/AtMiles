    ...
    public class ImportDataFromCvPartnerNewUserAndNewOfficeTest : EventSpecification<ImportDataFromCvPartner>
    {
        ...
        public override IEnumerable<Event> Expect()
        {
            var events = new List<Event>
                {
                    new EmployeeAdded(CompanyId, CompanyName,EmployeeId,NameService.GetName(_importData.FirstName, _importData.MiddleName, _importData.LastName),new Login(Constants.GoogleIdProvider, _importData.Email, string.Empty),_timestamp11, new Person(AdminId, NameService.GetName(AdminFirstName, AdminLastName)), _correlationId),
                    new EmployeeCreated(CompanyId, CompanyName, EmployeeId, new Login(Constants.GoogleIdProvider,_importData.Email, string.Empty), _importData.FirstName, _importData.MiddleName, _importData.LastName, _importData.DateOfBirth, _importData.Title, _importData.OfficeName, _importData.Phone,_importData.Email, null, null,_timestamp21, new Person(AdminId, NameService.GetName(AdminFirstName, AdminLastName)), _correlationId),
                    new Events.Import.ImportedFromCvPartner(CompanyId, CompanyName, EmployeeId, _importData.FirstName, _importData.MiddleName, _importData.LastName, _importData.DateOfBirth ,_importData.Email, _importData.Phone, _importData.Title, _importData.OfficeName, _importData.UpdatedAt, _importData.KeyQualifications, _importData.Technologies, _importData.Photo, _timestamp22, new Person(AdminId, Domain.Services.NameService.GetName(AdminFirstName, AdminLastName)), _correlationId)
                };
            return events;
        }
    }
